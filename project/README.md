...
1. **Train for 2 epochs on synthetic data**:
   ```bash
   python project/train.py --epochs 2 --synthetic true
   ```
   *This saves checkpoints to `project/checkpoints/` and logs TensorBoard telemetry to `project/logs/`.*

2. **Evaluate & Build the FAISS index**:
   ```bash
   python project/evaluate.py --checkpoint project/checkpoints/latest.pth --synthetic true
   ```
   *This builds the FAISS index database in `project/checkpoints/faiss_index.bin`, logs evaluation metrics (Precision@5, Recall@5, mAP), and saves visual plots (`tsne.png`, `umap.png`, `similarity_heatmap.png`) to `project/visualizations/`.*

3. **Perform a Query Retrieval Search**:
   ```bash
   python project/demo.py --checkpoint project/checkpoints/latest.pth --query_index 4 --synthetic true
   ```
...
Once the datasets are available:

1. Place your dataset files in `project/data/ben14k` or `project/data/dsrsid`.
2. Open `project/configs/config.yaml` and set `use_synthetic: false` and the correct dataset `name` (e.g. `ben14k` or `dsrsid`).
3. Run training:
   ```bash
   python project/train.py --config project/configs/config.yaml
   ```
...
1. Create your new model implementation inside `project/models/saber.py` exposing a class `SABER` matching the standard PyTorch `nn.Module` signature.
...
3. Update `config.yaml` or swap the import inside `train.py`, `evaluate.py`, and `demo.py` from `from project.models.rejepa import REJEPA` to `from project.models.saber import SABER`.
