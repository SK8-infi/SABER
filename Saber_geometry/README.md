...
Saber_geometry/
  ...
  models/
    ...
    Saber_geometry.py              # Core model combining all modules
...
python -m venv Saber_geometry/.venv

# Activate the virtual environment
# Windows (PowerShell):
.\Saber_geometry\.venv\Scripts\Activate.ps1
# Linux/macOS:
source Saber_geometry/.venv/bin/activate
```
...
python -m pip install -r Saber_geometry/requirements.txt
```
...
1. **Train for 2 epochs on synthetic data**:
   ```bash
   python Saber_geometry/train.py --epochs 2 --synthetic true
   ```
   *This saves checkpoints to `Saber_geometry/checkpoints/` and logs TensorBoard telemetry to `Saber_geometry/logs/`.*

2. **Evaluate & Build the FAISS index**:
   ```bash
   python Saber_geometry/evaluate.py --checkpoint Saber_geometry/checkpoints/latest.pth --synthetic true
   ```
   *This builds the FAISS index database in `Saber_geometry/checkpoints/faiss_index.bin`, logs evaluation metrics (Precision@5, Recall@5, mAP), and saves visual plots (`tsne.png`, `umap.png`, `similarity_heatmap.png`) to `Saber_geometry/visualizations/`.*

3. **Perform a Query Retrieval Search**:
   ```bash
   python Saber_geometry/demo.py --checkpoint Saber_geometry/checkpoints/latest.pth --query_index 4 --synthetic true
   ```
...
You can configure dataset settings directly inside `Saber_geometry/configs/config.yaml` or use **CLI overrides** at runtime to avoid editing config files on disk.
...
1. **Train on Sentinel-1 SAR modality for BEN-14K**:
   ```bash
   python Saber_geometry/train.py --dataset_name ben14k --modality s1 --data_dir c:/Github/SABER/Datasets/benv1_14k --epochs 5 --synthetic false
   ```

2. **Evaluate Cross-Modal retrieval (SAR ◄► Optical)**:
   ```bash
   python Saber_geometry/evaluate.py --checkpoint checkpoints/latest.pth --dataset_name ben14k --modality both --data_dir c:/Github/SABER/Datasets/benv1_14k --synthetic false
   ```

3. **Train on DSRSID (Gaofen-1) dataset**:
   ```bash
   python Saber_geometry/train.py --dataset_name dsrsid --data_dir c:/Github/SABER/Datasets/DSRSID/DSRSID-001.mat --epochs 5 --synthetic false
   ```
...
1. Create your new model implementation inside `Saber_geometry/models/saber.py` exposing a class `SABER` matching the standard PyTorch `nn.Module` signature.
...
3. Update `config.yaml` or swap the import inside `train.py`, `evaluate.py`, and `demo.py` from `from Saber_geometry.models.Saber_geometry import REJEPA` to `from Saber_geometry.models.saber import SABER`.
