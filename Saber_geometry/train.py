...
from Saber_geometry.utils.config import load_config
from Saber_geometry.utils.seed import set_seed
from Saber_geometry.utils.logger import setup_logger
from Saber_geometry.datasets.ben14k import BEN14KDataset
from Saber_geometry.datasets.dsrsid import DSRSIDDataset
from Saber_geometry.datasets.transforms import get_transforms
from Saber_geometry.models.Saber_geometry import REJEPA
from Saber_geometry.losses.saber_loss import SaberCombinedLoss
from Saber_geometry.trainer.trainer import Trainer

def main() -> None:
    parser = argparse.ArgumentParser(description="Train REJEPA-style Remote Sensing Image Retrieval System")
    parser.add_argument("--config", type=str, default="Saber_geometry/configs/config.yaml", help="Path to config file")
    ...
    criterion = SaberCombinedLoss(
        jaccard_weight=float(config.loss.jaccard_regression_weight),
        ranking_weight=float(config.loss.listwise_ranking_weight),
        ranking_temp_s=float(config.loss.ranking_temp_s),
        ranking_temp_p=float(config.loss.ranking_temp_p),
