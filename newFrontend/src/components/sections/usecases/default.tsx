...
/* ── Use case data ─────────────────────────────────────────────
   All images are publicly available Sentinel-1/2 open-data tiles
   served via the Copernicus Open Access Hub CDN or ESA EO Browser
   static previews.
────────────────────────────────────────────────────────────── */
const USE_CASES = [
  {
    ...
    // Wikimedia Commons — ESA Copernicus Sentinel-1 SAR flood image, Valencia 2024
    ...
    // Wikimedia Commons — Sentinel-2 optical, Valencia region
  ...
  },
  {
    ...
    sarLabel: "Sentinel-1 SAR · Central-eastern Brazil farmland (ESA)",
  ...
  },
  {
    ...
    sarLabel: "Sentinel-2 Optical · Toronto, Canada (ESA/Copernicus)",
    opticalLabel: "Sentinel-2 Optical · Los Angeles, USA (ESA/Copernicus)",
...
/* ── Image pair card ──────────────────────────────────────── */
function ImagePair({
  sarImg, opticalImg, sarLabel, opticalLabel
}: {
    ...
    <div className="grid grid-cols-2 gap-2 rounded-xl overflow-hidden">
      ...
      <div className="relative flex flex-col gap-1.5">
        <div className="relative aspect-square rounded-lg overflow-hidden bg-zinc-900 border border-border/40">
          ...
          <span className="absolute bottom-2 left-2 text-[9px] font-bold font-mono uppercase tracking-wider text-white/80 bg-black/60 px-1.5 py-0.5 rounded">
      ...
      <div className="relative flex flex-col gap-1.5">
        ...
        <div className="relative aspect-square rounded-lg overflow-hidden bg-zinc-900 border border-[#FBBA72]/20">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
          <span className="absolute bottom-2 left-2 text-[9px] font-bold font-mono uppercase tracking-wider text-[#FBBA72] bg-black/60 px-1.5 py-0.5 rounded">
...
/* ── Synthetic fallback patterns when images fail to load ─── */
  ...
  return (
    <svg viewBox="0 0 120 120" className="w-full h-full opacity-60">
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <rect
          key={`b${i}`}
          x={10 + i * 13} y={20 + (i % 3) * 30}
          width={8 + (i % 3) * 4} height={8 + (i % 2) * 6}
          fill={`hsl(0,0%,${50 + i * 5}%)`}
          opacity={0.8}
        />
      ))}
...
function SyntheticOptical() {
  return (
    <svg viewBox="0 0 120 120" className="w-full h-full">
      ...
      <rect x="0" y="85" width="120" height="5" fill="#4a7a4a" opacity="0.5" />
    ...
    <Section>
      <div className="max-w-container mx-auto flex flex-col gap-12 sm:gap-16">
        ...
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <h2 className="text-3xl font-semibold leading-tight sm:text-5xl sm:leading-tight max-w-[640px]">
            Where SABER changes the game
          </h2>
          <p className="text-muted-foreground max-w-[580px] text-base leading-relaxed">
            SAR radar sees through clouds, smoke, and darkness. Optical imagery is visually
            interpretable. SABER bridges both — enabling cross-sensor retrieval in under 30ms.
          </p>
        </div>
          ...
          {USE_CASES.map((uc) => (
            <button
              ...
              className={cn(
                ...
                activeId === uc.id
                  ? "border-[#FBBA72]/50 bg-[#FBBA72]/10 text-[#FBBA72]"
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-16 items-center">
          ...
          <div className="flex flex-col gap-6">
            <span className={cn(
              "self-start text-xs font-bold uppercase tracking-widest border rounded-full px-3 py-1",
              active.tagColor
            )}>
              {active.tag}
            </span>
            ...
            <div className="flex flex-col gap-3 rounded-xl border border-border/50 bg-muted/10 p-5">
              ...
              <ol className="flex flex-col gap-2 text-sm text-muted-foreground">
                <li className="flex gap-2">
                  ...
                  SAR image encoded by wavelength-conditioned DOFA ViT backbone
                </li>
                <li className="flex gap-2">
                  <span className="font-bold text-foreground shrink-0">2.</span>
                  CFM latent bridge transports SAR embedding → optical hypersphere
                </li>
                <li className="flex gap-2">
                  <span className="font-bold text-foreground shrink-0">3.</span>
                  FAISS cosine search retrieves Top-K optical matches in &lt;1ms
                </li>
              </ol>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={cn("text-sm font-semibold font-mono", active.resultColor)}>
                {active.result}
              </span>
            </div>
          </div>
